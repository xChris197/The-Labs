...
public class SCR_KeypadButton2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (distance < 2f && SCR_PlayerCasting.hitTarget.CompareTag("Keypad Button 2"))
        {
        ...
        if (distanceTwo < 2f && SCR_PlayerCastingTwo.hitTarget.CompareTag("Keypad Button 2"))
        {
        ...
        if (distance < 2f && SCR_PlayerCasting.hitTarget.CompareTag("Keypad Button 2") && (Input.GetButtonDown(interactOne)))
        {
            inputSound.Play();
            keypadScript.AddPlayerInput(keypadValue);
        }
        else if (distanceTwo < 2f && SCR_PlayerCastingTwo.hitTarget.CompareTag("Keypad Button 2") && Input.GetButtonDown(interactTwo))
        {
            inputSound.Play();
            keypadScript.AddPlayerInput(keypadValue);
        }

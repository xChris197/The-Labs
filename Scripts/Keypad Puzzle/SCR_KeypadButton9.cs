...
public class SCR_KeypadButton9 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (distance < 2f && SCR_PlayerCasting.hitTarget.CompareTag("Keypad Button 9"))
        {
        ...
        if (distanceTwo < 2f && SCR_PlayerCastingTwo.hitTarget.CompareTag("Keypad Button 9"))
        {
        ...
        if (distance < 2f && SCR_PlayerCasting.hitTarget.CompareTag("Keypad Button 9") && (Input.GetButtonDown(interactOne)))
        {
            inputSound.Play();
            keypadScript.AddPlayerInput(keypadValue);
        }
        else if (distanceTwo < 2f && SCR_PlayerCastingTwo.hitTarget.CompareTag("Keypad Button 9") && Input.GetButtonDown(interactTwo))
        {
            inputSound.Play();
            keypadScript.AddPlayerInput(keypadValue);
        }

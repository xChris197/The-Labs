...
public class SCR_KeypadButton5 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (distance < 2f && SCR_PlayerCasting.hitTarget.CompareTag("Keypad Button 5"))
        {
        ...
        if (distanceTwo < 2f && SCR_PlayerCastingTwo.hitTarget.CompareTag("Keypad Button 5"))
        {
        ...
        if (distance < 2f && SCR_PlayerCasting.hitTarget.CompareTag("Keypad Button 5") && (Input.GetButtonDown(interactOne)))
        {
            inputSound.Play();
            keypadScript.AddPlayerInput(keypadValue);
        }
        else if (distanceTwo < 2f && SCR_PlayerCastingTwo.hitTarget.CompareTag("Keypad Button 5") && Input.GetButtonDown(interactTwo))
        {
            inputSound.Play();
            keypadScript.AddPlayerInput(keypadValue);
        }
